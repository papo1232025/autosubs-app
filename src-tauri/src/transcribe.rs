...
/// 日本語テキストを 12 文字以内で自然に改行する
/// - 「。」で文を分割（ただし最終出力からは除去）
/// - 行の最大長は 12
/// - 助詞「は/が/を/に/で/と/も/へ/や/から/ので」で優先的に改行
/// - 1文字だけの行ができないよう調整
pub fn format_japanese_smart(input: &str, max_len: usize) -> String {
    let mut result = String::new();

    // STEP 1: 「。」で優先的に文を分ける（ただし「。」は消す）
    ...
    let particles = ["は", "が", "を", "に", "で", "と", "も", "へ", "や", "から", "ので"];
        ...
        for &ch in chars.iter() {
            ...
            if line.chars().count() >= max_len {
                ...
                if let Some(bp) = break_pos {
                    ...
                    // 1文字行を避ける調整
                    if line.chars().count() - cut_len <= 1 {
                        cut_len -= 1;
                    }

                    let cut: String = line.chars().take(cut_len).collect();
                    let rest: String = line.chars().skip(cut_len).collect();
...
// これをファイル上部に追加
fn wrap_japanese_smart(text: &str, max_chars: usize) -> String {
    let mut result = String::new();
    let mut line = String::new();
    let mut count = 0;
...
fn split_segment_by_char_limit(seg: &Segment, max_chars: usize) -> Vec<Segment> {
    ...
    for ch in text.chars() {
        ...
        if count >= max_chars {
            let end_offset = char_index as f64 * sec_per_char;

            result.push(Segment {
                start: seg.start + ((char_index - count) as f64 * sec_per_char),
                end: seg.start + end_offset,
                text: current.clone(),
